...
malvin({
    ...
    category: "ai",
    react: "🤖",
    use: ".cyberia <your query>",
    filename: __filename,
}, async (malvin, mek, m, { from, args, q, reply }) => {
    try {
        // Vérification de l'entrée utilisateur
        if (!q) return reply("⚠️ Please provide a query for cyberia Ai.\n\nExample:\n.cyberia What is AI?");
        ...
        if (!response || !response.data || !response.data.result) {
            return reply("❌ No response received from the cyberia API. Please try again later.");
        }
        ...
        // Image AI à envoyer
        const ALIVE_IMG = 'https://files.catbox.moe/lvomei.jpg'; // Remplacez par l'URL de votre image AI

        // Légende avec des informations formatées
        const formattedInfo = `🤖 *Cyberia's Response:*\n\n${gptResponse}`;
...
malvin({
    ...
    try {
        ...
        // AI image to attach
        const AI_IMG = 'https://files.catbox.moe/lvomei.jpg'; // Replace with a valid image URL
