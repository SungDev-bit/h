...
const getContextInfo = (sender, config) => ({
  ...
  forwardedNewsletterMessageInfo: {
    ...
    newsletterName: config.OWNER_NAME || 'Sung Tech🪀',
    serverMessageId: 143,
  },
});
...
const sendMenuMessage = async (malvin, from, mek, caption, imageUrl, contextInfo) => {
  if (!isValidUrl(imageUrl)) {
    ...
    imageUrl = 'https://files.catbox.moe/lvomei.jpg'; // Fallback image
  }
  ...
  malvin(
    ...
    async (malvin, mek, m, { from, sender, reply, isCreator }) => {
      try {
        ...
        const imageUrl = config.MENU_IMAGES?.[menu.imageKey] || config.MENU_IMAGE_URL || 'https://files.catbox.moe/lvomei.jpg';
